import { useMutation } from '@redwoodjs/web'
import { toast } from '@redwoodjs/web/toast'
import { Link, routes, navigate } from '@redwoodjs/router'

const DELETE_PLANT_VARIETY_MUTATION = gql`
  mutation DeletePlantVarietyMutation($id: String!) {
    deletePlantVariety(id: $id) {
      id
    }
  }
`

const jsonDisplay = (obj) => {
  return (
    <pre>
      <code>{JSON.stringify(obj, null, 2)}</code>
    </pre>
  )
}

const timeTag = (datetime) => {
  return (
    <time dateTime={datetime} title={datetime}>
      {new Date(datetime).toUTCString()}
    </time>
  )
}

const checkboxInputTag = (checked) => {
  return <input type="checkbox" checked={checked} disabled />
}

const PlantVariety = ({ plantVariety }) => {
  const [deletePlantVariety] = useMutation(DELETE_PLANT_VARIETY_MUTATION, {
    onCompleted: () => {
      toast.success('PlantVariety deleted')
      navigate(routes.plantVarieties())
    },
  })

  const onDeleteClick = (id) => {
    if (confirm('Are you sure you want to delete plantVariety ' + id + '?')) {
      deletePlantVariety({ variables: { id } })
    }
  }

  return (
    <>
      <div className="rw-segment">
        <header className="rw-segment-header">
          <h2 className="rw-heading rw-heading-secondary">
            PlantVariety {plantVariety.id} Detail
          </h2>
        </header>
        <table className="rw-table">
          <tbody>
            <tr>
              <th>Id</th>
              <td>{plantVariety.id}</td>
            </tr>
            <tr>
              <th>Name</th>
              <td>{plantVariety.name}</td>
            </tr>
            <tr>
              <th>Description</th>
              <td>{plantVariety.description}</td>
            </tr>
            <tr>
              <th>Seeds source id</th>
              <td>{plantVariety.seedsSourceId}</td>
            </tr>
            <tr>
              <th>Days to maturity</th>
              <td>{plantVariety.daysToMaturity}</td>
            </tr>
          </tbody>
        </table>
      </div>
      <nav className="rw-button-group">
        <Link
          to={routes.editPlantVariety({ id: plantVariety.id })}
          className="rw-button rw-button-blue"
        >
          Edit
        </Link>
        <a
          href="#"
          className="rw-button rw-button-red"
          onClick={() => onDeleteClick(plantVariety.id)}
        >
          Delete
        </a>
      </nav>
    </>
  )
}

export default PlantVariety
